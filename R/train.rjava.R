...
exh.train <- function(data, predictors, response,
		      ...
		      test.thresholds.predictors = NULL, # ignored if filter.randomForest is a list
		      directions = NULL,
  ...
  test.thresholds <- test.thresholds.predictors
  # Remove nas
  if (na.rm == "all") {
  ...
  if (is.list(filter.randomForest)) {
    ...
    if (is.null(test.thresholds)) {
      for (pred in all.predictors) {
        predictor.roc <- pROC::roc(data[[response]], data[[pred]], levels = levels, direction = directions[[pred]])
        ...
        possible.thresholds[[pred]] <- coords.vector
       	...
       	for (pred in all.predictors) {
    		...
    		if (is.null(test.thresholds[[pred]])) {
    			possible.thresholds[[pred]] <- coords.vector
    		} else { 
